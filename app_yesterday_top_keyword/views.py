...
def chart_cate_topword_y(request):
    return render(request, 'app_yesterday_top_keyword/chart_topword_yesterday.html')


# read df
df_topkey = pd.read_csv(
    'app_yesterday_top_keyword/dataset/cna_topword_yesterday.csv', sep=',')
...
@csrf_exempt
def api_get_cate_topword_y(request):
    ...
    chart_data, wf_pairs = get_category_topword_y(cate, topk)
...
def get_category_topword_y(cate, topk=10):
